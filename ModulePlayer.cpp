...
bool ModulePlayer::Start()
{
	...
	shipcollider= App->collision->AddCollider({ position.x, position.y, 32, 14 }, COLLIDER_PLAYER, App->player);
...
update_status ModulePlayer::Update()
{
	...
	shipcollider->SetPos(position.x, position.y);
...
void ModulePlayer::OnCollision(Collider* c1, Collider* c2)
{
	if (c1->CheckCollision(c2->rect))
	{
		App->player->Disable();
		App->render->Blit(graphics, position.x, position.y, &(current_animation->GetCurrentFrame()));
		App->particles->AddParticle(App->particles->explosion, position.x + 16, position.y,COLLIDER_NONE,300);
